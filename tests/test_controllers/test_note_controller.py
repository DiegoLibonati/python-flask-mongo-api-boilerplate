...
class TestTestErrorController:
    ...
    def test_raises_internal_api_error(self, app: Flask) -> None:
        with app.app_context(), pytest.raises(InternalAPIError):
            controller_test_error()

    @pytest.mark.unit
    def test_error_has_correct_code(self, app: Flask) -> None:
        with app.app_context(), pytest.raises(InternalAPIError) as exc_info:
            controller_test_error()
...
class TestCreateNoteController:
    ...
    def test_returns_201_when_created(self, app: Flask) -> None:
        ...
        with (
            app.test_request_context("/api/v1/notes/", method="POST", json={"name": "test_note"}),
            patch("src.controllers.note_controller.NoteService.add_note", return_value=mock_result),
        ):
    ...
    def test_raises_conflict_when_name_already_exists(self, app: Flask) -> None:
        with (
            app.test_request_context("/api/v1/notes/", method="POST", json={"name": "existing"}),
            patch(
    ...
    def test_raises_validation_error_for_empty_name(self, app: Flask) -> None:
        with (
            app.test_request_context("/api/v1/notes/", method="POST", json={"name": ""}),
            pytest.raises(ValidationAPIError),
        ):
            create_note()
...
class TestGetNotesController:
    ...
    def test_returns_200(self, app: Flask) -> None:
        with app.app_context(), patch("src.controllers.note_controller.NoteService.get_all_notes", return_value=[]):
    ...
    def test_response_data_is_list(self, app: Flask) -> None:
        ...
        with app.app_context(), patch("src.controllers.note_controller.NoteService.get_all_notes", return_value=notes):
            response, status = get_notes()
            data: dict[str, Any] = response.get_json()
    ...
    def test_service_is_called_once(self, app: Flask) -> None:
        with (
            app.app_context(),
            patch("src.controllers.note_controller.NoteService.get_all_notes", return_value=[]) as mock_get,
        ):
            get_notes()
...
class TestDeleteNoteController:
    ...
    def test_returns_200_when_deleted(self, app: Flask) -> None:
        ...
        with app.app_context(), patch("src.controllers.note_controller.NoteService.delete_note_by_id"):
    ...
    def test_raises_validation_error_for_invalid_id(self, app: Flask) -> None:
        with app.app_context(), pytest.raises(ValidationAPIError) as exc_info:
    ...
    def test_service_called_with_objectid(self, app: Flask) -> None:
        _id: ObjectId = ObjectId()
        with app.app_context(), patch("src.controllers.note_controller.NoteService.delete_note_by_id") as mock_delete:
